...
class SlicesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        i, j = slc
        sample['image'] = torch.from_numpy(self.data[i]['image'][j,:,:]).type(torch.cuda.FloatTensor).unsqueeze(0)
        sample['seg'] = torch.from_numpy(self.data[i]['seg'][j,:,:]).type(torch.cuda.LongTensor).unsqueeze(0)
